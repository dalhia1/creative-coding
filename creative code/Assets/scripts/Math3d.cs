...
public class Math3d
{
	...
	public static void Init()
	{
		...
		tempChild = (new GameObject("MathFor3d_TempChild")).transform;
		tempParent = (new GameObject("MathFor3d_TempParent")).transform;
	...
	private float Xroot(float a, float x) {
	...
	//Solve cubic equation according to Cardano. Alternate version.
	//Source: https://www.codeproject.com/Articles/798474/To-Solve-a-Cubic-Equation
	private void SolveCubic2(out int res, out float x1real, out float x2real, out float x3real, float a, float b, float c, float d) {
		...
		if (a1 != 0) {
			...
			p = -(a * a / 3f) + b;
			q = (2f / 27f * a * a * a) - (a * b / 3f) + c;
			d = q * q / 4f + p * p * p / 27f;

			if (Mathf.Abs(d) < Mathf.Pow(10f, -11f)) {
			...
			if (d > 1e-20) {
				...
				u = Xroot(-q / 2f + Mathf.Sqrt(d), 3f);
				v = Xroot(-q / 2f - Mathf.Sqrt(d), 3f);
				x1real = u + v - a / 3f;
				x2real = -(u + v) / 2f - a / 3f;
				x3real = x2real;
				res = 1;
			...
			if (d < -1e-20) {
				...
				r = Mathf.Sqrt(-p * p * p / 27f);
				alpha = Mathf.Atan(Mathf.Sqrt(-d) / -q * 2f);

				if (q > 0) {

					alpha = 2f * Mathf.PI - alpha;
				}

				x1real = Xroot(r, 3f) * (Mathf.Cos((6f * Mathf.PI - alpha) / 3f) + Mathf.Cos(alpha / 3f)) - a / 3f;
				x2real = Xroot(r, 3f) * (Mathf.Cos((2f * Mathf.PI + alpha) / 3f) + Mathf.Cos((4f * Mathf.PI - alpha) / 3f)) - a / 3f;
				x3real = Xroot(r, 3f) * (Mathf.Cos((4f * Mathf.PI + alpha) / 3f) + Mathf.Cos((2f * Mathf.PI - alpha) / 3f)) - a / 3f;
	...
	dir = MathFor3d.SetVectorLength(dir, dir.magnitude * scale);
